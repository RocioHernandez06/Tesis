...
Esto demuestra que muchos métodos clásicos basados en convexidad y separabilidad @sec-funcion_CD resultan inaplicables en condiciones realistas.
...
El tensor $y = (y_{ij})_{(i,j) \in \mathcal{I} \times \mathcal{J}} \in \mathbb{R}_+^{m \times n}$ describe la **estructura de asignación de suministros desde almacenes candidatos a localidades de demanda**, y pertenece a un espacio vectorial convexo, cerrado @sec-conjuntosE y de dimensión finita.
...
donde $M > 0$ es una constante suficientemente grande (por ejemplo, $M = \sum_{j \in \mathcal{J}} d_j$). La restricción @eq-y_ij es una formulación *big-M* estándar @sec-bigM que modela la implicación lógica:
...
es un **poliedro convexo** @sec-conjuntosE (posiblemente vacío).
