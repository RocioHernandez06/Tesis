...
La consecuencia directa es que condiciones de optimalidad de tipo KKT **no garantizan** optimalidad global (@LewisOverton2013).


Aquí, $d_j > 0$ representa la demanda de la zona $j$ y $M$ es un parámetro de gran magnitud. La tercera restricción impone un vínculo lógico: si $x_i = 0$, entonces $s_i = 0$ y, en consecuencia, $y_{ij} = 0$ para todo $j \in \mathcal{J}$.

Este acoplamiento, combinado con la naturaleza **binaria** de las variables $x_i$, implica que el conjunto factible $\mathcal{X}$ **no es convexo** como subconjunto del espacio $\mathbb{R}^{m+mn+m}$, donde las componentes corresponden, respectivamente, a las variables de apertura ($x$), flujos ($y$) e inventario ($s$).

La no convexidad se debe exclusivamente a la **discreción** de las variables $x_i$: el promedio de dos puntos factibles con configuraciones distintas de infraestructura no es factible, ya que viola la condición $x_i \in \{0,1\}$. Esta propiedad, la ausencia de convexidad en el conjunto factible debido a variables enteras, es bien conocida en la teoría de optimización mixta ver @Bertsekas1999.
...
Debido a esta no convexidad, el problema **no pertenece a la clase de optimización convexa**, y en consecuencia **no es posible garantizar optimalidad global mediante condiciones de optimalidad basadas en el análisis convexo clásico**. En particular, las condiciones de Karush–Kuhn–Tucker **no están definidas para el problema completo**, ya que presuponen un conjunto factible convexo y variables continuas.
...
Aunque la parte en variables continuas es lineal, el término $f_i(x_i)$ genera una **no linealidad discreta**; más aún, en la práctica humanitaria pueden existir economías de escala, accesibilidad variable o costos de apertura no lineales, reforzando la complejidad global del problema, entendida como la combinación de no convexidad, discreción y no linealidad que lo sitúa en la clase de problemas MINLP, de resolución computacionalmente exigente (@Grossmann2002).
...
- $\mathcal{J}$: el conjunto de **zonas geográficas afectadas**.
...
El vector $x = (x_i)_{i \in \mathcal{I}} \in \{0,1\}^m$ codifica una **configuración de infraestructura** del sistema logístico. Este vector pertenece al conjunto discreto $\{0,1\}^m$, que es no convexo, finito y tiene cardinalidad $2^m$.
...
que representa la cantidad de unidades asignadas desde la instalación $i$ a la zona de demanda $j$.

El tensor $y = (y_{ij})_{(i,j) \in \mathcal{I} \times \mathcal{J}} \in \mathbb{R}_+^{m \times n}$ describe la **estructura de asignación de suministros desde almacenes candidatos a zonas de demanda**, y pertenece a un espacio vectorial convexo, cerrado y de dimensión finita.
...
(i) que la demanda en cada zona $j \in \mathcal{J}$ sea satisfecha y 
...
d_j\in\mathbb{R}_{++}
$$
la **demanda esperada** en la zona $j$ durante el horizonte considerado. Adicionalmente definimos el vector $d=(d_j)_{j\in\mathcal{J}}$. En la formulación determinista de referencia usamos $d_j$ como estimador puntual (p. ej. esperanza), con la advertencia explícita de que su uso directo puede producir soluciones frágiles en colas de distribución como lo argumenta @Snyder2006. Notación:
...
Dado el conjunto de almacenes potenciales $\mathcal{I}$, el conjunto de zonas de demanda $\mathcal{J}$, y los parámetros exógenos $d_j > 0$, $s_i^{\max} \in \mathbb{R}_+ \cup \{+\infty\}$, y una constante $M > 0$ suficientemente grande (por ejemplo, $M = \sum_{j \in \mathcal{J}} d_j$), el **espacio factible** del modelo se define como:
...
Consideremos un producto con los siguientes parámetros: demanda anual $d = 1200$ unidades/año, costo fijo de pedido $K = 100$ USD/pedido, y costo unitario de mantenimiento $h = 5$ MXN/unidad/año. El tamaño óptimo de pedido es:
$$
...
$$
es decir, **el 97 % de los escenarios conducen a faltantes severos**. Si, en cambio, se adopta un enfoque de *newsvendor* con un costo de faltante conservador $p = 100~\text{MXN/unidad}$ (reflejando impacto indirecto), se obtiene:
$$
...
Sea $\tilde{D}$ una variable aleatoria no negativa que representa la demanda durante el periodo de planificación, definida sobre un espacio de probabilidad $(\Omega, \mathcal{F}, \mathbb{P})$. Denotemos por $Q \geq 0$ la cantidad de inventario pre-posicionado (decisión de primer estadio). El costo total asociado a $Q$ se descompone en:
...
Supongamos $\tilde{D} \sim \text{Lognormal}(\mu = 6.8, \sigma = 0.6)$, lo que implica $\mathbb{E}[\tilde{D}] \approx 1200$, $\text{Desv. estándar} \approx 800$. Considérese $\phi(z) = z^{1.5}$, $c = 10~\text{MXN/unidad}$, $h = 2~\text{MXN/unidad}$, y $\beta = 0.95$.
...
#### Ejemplo paso a paso: caso con $\phi(z) = z^2$
...
En particular, si $\phi(z) = p z$ (lineal), la condición de optimalidad se reduce a:
$$
...
#### Ejemplo paso a paso: calibración de $\beta_C$ bajo $\psi(z) = z^2$
...
Sea $\tilde{D} \sim \text{Uniforme}(1000, 2000)$, $c = 12~\text{MXN/unidad}$, $h = 4~\text{MXN/unidad}$, y $\psi(z) = z^2$. Entonces:
$$
...
#### Ejemplo paso a paso: impacto no lineal de $L$ en la varianza de la demanda
...
#### Ejemplo paso a paso: demostración del efecto descendente
...
- $K = 1000~\text{MXN}$ (costo fijo "administrativo" asignado arbitrariamente).
...
- $c = 24~\text{MXN/unidad}$,
- $p = 25~\text{MXN/unidad}$ (penalización marginal apenas positiva),
- $K = 5000~\text{MXN}$ (costo fijo inflado por burocracia percibida).
