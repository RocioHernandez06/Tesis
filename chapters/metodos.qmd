...
1. $f$ es continuamente diferenciable (ver @sec-funcion_CD) en un conjunto abierto que contiene al conjunto de nivel $\mathcal{L} = \{x \in \mathbb{R}^n : f(x) \le f(x_0)\}$,
...
- Si además $f$ es cuadrática y fuertemente convexa, BFGS converge en a lo sumo $n$ pasos, generando direcciones conjugadas (ver @sec-funcion_CD) respecto al Hessiano exacto. Nos referimos a que, si 
$$
...
Sea $f : \mathbb{R}^n \to \mathbb{R}$ una función dos veces continuamente diferenciable (ver @sec-funcion_CD). En cada iteración $k$, se dispone de los incrementos
...
donde $f: \mathbb{R}^n \to \mathbb{R}$ es continuamente diferenciable (ver @sec-funcion_CD) en un conjunto abierto que contiene al conjunto factible
...
> La función $f$ es continuamente diferenciable (ver @sec-funcion_CD) en un conjunto abierto que contiene a $\Omega$, y su gradiente $\nabla f$ es **Lipschitz continuo** en $\Omega$; es decir, existe una constante $L > 0$ tal que  
> $$
...
Aquí, $f : \mathbb{R}^n \to \mathbb{R}$ es continuamente diferenciable (ver @sec-funcion_CD), y el conjunto factible
...
> $f$ es dos veces continuamente diferenciable (ver @sec-funcion_CD) en un conjunto abierto que contiene $\{Q \ge \mathbf{1}\}$, y es **estrictamente convexa**, es decir:
> $$
...
Este modelo permite cuantificar cómo $m$ regula el equilibrio entre **rapidez de convergencia** (ver @sec-funcion_CD) y **costo computacional por iteración**, especialmente relevante en problemas de gran escala.
...
2. La corrección iterativa basada en pares $(s_k, y_k)$ que reduce el mal condicionamiento de la Hessiana aproximada,
...
### Comparación numérica con otros métodos: gradiente descendente, Newton inexacto y BFGS sin restricciones {#sec-comparacion}
