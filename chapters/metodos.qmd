...
2. $\mathcal{L}$ es compacto @sec-compacto,
...
- Si $f$ es dos veces continuamente diferenciable y su Hessiano $\nabla^2 f$ es Lipschitz-continuo @sec-funcion_CD en una vecindad de un minimizador $x^\star$, y la búsqueda lineal satisface las condiciones de Wolfe, entonces la sucesión $\{x_k\}$ cumple
...
En solo dos iteraciones el algoritmo reduce el error residual en más de un orden de magnitud, demostrando la **convergencia superlineal** @sec-convergenciasuperlineal del método BFGS para funciones cuadráticas. Este comportamiento se mantiene en problemas no cuadráticos siempre que el condicionamiento sea razonable y la búsqueda lineal sea suficientemente precisa.
...
sea de descenso. Además, si $f$ es una función cuadrática fuertemente convexa @sec-funcion_CD, cualquier método de la familia restringida de Broyden converge en a lo sumo $n$ pasos, generando direcciones conjugadas @sec-funcion_CD respecto al Hessiano exacto $\nabla^2 f$ @DennisMore1977, Sección 5.
...
1. **No expansividad**
   ...
   Por tanto, $P_\Omega$ es una aplicación **Lipschitz continua (@sec-funcion_CD) con constante 1**, lo que implica estabilidad numérica.
