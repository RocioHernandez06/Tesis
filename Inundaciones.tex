...
Una estrategia robusta impone las \textbf{condiciones de Wolfe}
((\textbf{NocedalWright20062006?})):
...
convergencia \textbf{superlineales} bajo hipótesis razonables (Nocedal
\& Wright, 2006).
...
del BFGS bajo las hipótesis de suavidad y curvatura positiva (Byrd
et~al. (1995); Zhu et~al. (1997)), y constituye uno de los métodos
estándar para problemas de gran escala con restricciones tipo caja.
...
estacionariedad proyectada \(\nabla_\Omega f(\bar{x}) = 0\). (Ver
Nocedal y Wright (2006); Byrd et~al. (1995).)
...
El \textbf{resultado fundamental de convergencia global} (Byrd, Lu,
Nocedal y Zhu, 1995) establece que, bajo hipótesis estándar de
regularidad y condiciones adecuadas de búsqueda lineal, la sucesión
generada por el algoritmo posee puntos límite que satisfacen las
\textbf{condiciones necesarias de optimalidad de Karush--Kuhn--Tucker
(KKT)}. Dicho de otra forma, el algoritmo converge ---en el sentido de
subsecuencias--- hacia el conjunto de puntos estacionarios proyectados.
...
un punto \$ x\^{}\star \in \Omega \$ que satisface las
\textbf{condiciones KKT} del problema (3.60).
\end{quote}
...
\item
  ...
  \] es decir, \$ d\_k \$ es una dirección de descenso o nula solo en
  puntos estacionarios.
\item
  \textbf{Aplicación del lema de Zoutendijk.}\\
  El análisis estándar (Zoutendijk, 1970; Nocedal \& Wright, 2006)
  implica que \[
  \sum_{k=0}^\infty \frac{(\nabla f(x_k)^\top d_k)^2}{\|d_k\|^2} < \infty. \tag{3.69}
  \] Dado que el numerador está acotado inferiormente por una constante
  positiva cuando \$ \textbar{}\nabla\_\Omega f(x\_k)\textbar{} \$ no
  tiende a cero, se concluye que \$ \textbar d\_k\textbar{} \to 0 \$.\\
...
\textbf{Evolución de L-BFGS-B (memoria limitada \$ m = 2 \$):} - Desde
\(x_0 = (1,1)^\top\), se obtiene \(x_1 = (3,1)\), \(x_2 = (2.5,1)\),
\(x_3 = (2.25,1)\), \(x_4 = (2.125,1)\), hasta estabilizarse en
\(x_k \to (2,1)\). - Para cada iteración: \[
