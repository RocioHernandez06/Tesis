...
fundamentales. Como se demostró en la Sección~\ref{sec-extension}, la
incorporación del inventario de seguridad parametrizado
\(Z_\alpha \sigma_L\) y la penalización explícita por faltantes mediante
el término \(\beta C\) transforman la función de costo total en una
expresión no convexa y no separable:
...
conjuntos), constituye una contribución original que permite
internalizar la incertidumbre sin recurrir a formulaciones estocásticas
complejas, preservando así la trazabilidad analítica y la eficiencia
computacional.
...
proyección), justificando su elección como motor de optimización para
los subproblemas continuos.
...
Urbina logra cobertura del \(100\%\) de la población afectada (\(7,407\)
...
En síntesis, esta investigación contribuye a la literatura de
optimización aplicada y logística humanitaria mediante la integración
coherente de tres elementos:

\begin{enumerate}
\def\labelenumi{(\roman{enumi})}
\item
  una formulación teórica rigurosa que extiende el EOQ clásico para
  incorporar incertidumbre parametrizada sin sacrificar trazabilidad
  analítica;
\item
...
Como trabajo futuro, se propone extender el modelo actual en tres
direcciones complementarias con fundamentación teórica sólida: primero,
incorporar una formulación estocástica multiperiodo que considere la
dinámica temporal de la demanda durante la fase de respuesta, superando
así la simplificación de horizonte único adoptada en esta investigación
mediante la integración de técnicas de programación estocástica con
recurrencia, tal como lo plantea Shapiro, Dentcheva, y Ruszczyński
(2021); segundo, integrar variables climáticas predictivas (como
pronósticos de precipitación a \(72\) horas) para habilitar un sistema
de reoptimización en tiempo real que ajuste las decisiones logísticas
conforme evoluciona el evento hidrometeorológico,siguiendo los marcos de
optimización robusta adaptable desarrollados por Bertsimas y Thiele
(2006); y tercero, escalar la metodología a nivel estatal en Chiapas
