...
\item
  ...
  y convexo (Bazaraa et al., 2013).
\item
  \textbf{Conjunto cerrado en dimensión finita}: En \(\mathbb{R}^n\)
  dotado de la topología euclidiana, un conjunto es \textbf{cerrado} si
  contiene todos sus puntos límite. Todo subespacio vectorial de
  \(\mathbb{R}^n\) es cerrado, y en dimensión finita, los conjuntos
  cerrados y acotados son compactos (teorema de Heine--Borel).
\end{itemize}
...
\item
  ...
  \(Q \in \mathbb{R}^{n \times n}\) simétrica. Si \(Q\) es constante,
  entonces el Hessiano \(\nabla^2 f(x) = Q\) es constante.
\item
...
\item
  ...
  \(M > 0\) suficientemente grande, usada para modelar
  \(x = 0 \Rightarrow y = 0\).
\end{itemize}

\section{Teoría de probabilidad y variables
aleatorias}\label{teoruxeda-de-probabilidad-y-variables-aleatorias}

\begin{itemize}
\tightlist
\item
  \textbf{Variable aleatoria (v.a.) absolutamente continua}: aquella
  cuya distribución tiene densidad \(f_X\) respecto a la medida de
  Lebesgue: \(\mathbb{P}(X \in A) = \int_A f_X(x)\,dx\).
\item
...
\item
  ...
  otra (por ejemplo, \(a \ll b\) significa \(a/b \approx 0\) en contexto
  numérico).
\end{itemize}
...
\item
  ...
  calificación de restricciones. No garantizan optimalidad global en
  problemas no convexos (Lewis y Overton, 2013).
\item
  \textbf{\(\mathbb{E}[d_j]\)}: valor esperado de la demanda en la
  localidad \(j\).
\end{itemize}
