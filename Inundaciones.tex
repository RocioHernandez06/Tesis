...
en \((0,\infty)\), pero no es convexa globalmente
...
separabilidad Sección~\ref{sec-funcion_CD} resultan inaplicables en
...
tanto, \(\mathcal{X}\) no es convexo. ∎ \{\#sec-noconvexidad\}
\end{quote}
...
\textbf{?@sec-noconvexidad}). Esta no convexidad, junto con la
...
vectorial convexo, cerrado Sección~\ref{sec-conjuntosE} y de dimensión
finita.
...
Ecuación~\ref{eq-y_ij} es una formulación \emph{big-M} estándar
...
es un \textbf{poliedro convexo} Sección~\ref{sec-conjuntosE}
(posiblemente vacío).
...
Sección~\ref{sec-extension} y derivar su expresión determinista efectiva
en la Sección~\ref{sec-derivacion}, es necesario establecer las
propiedades analíticas fundamentales del problema de optimización
unidimensional resultante. En esta sección, se analiza rigurosamente el
problema \(\min_{Q \geq 1} f(Q)\), demostrando la existencia y unicidad
del minimizador, así como el comportamiento asintótico de la función
...
que corresponde a la ecuación \textbf{?@eq-Z}(q) en este documento.
...
no constante de la ecuación \textbf{?@eq-Z}(q) como:
