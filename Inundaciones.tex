...
Diseñar e implementar un modelo de optimización \textbf{determinista
robusto} para la gestión integrada de la ubicación de almacenes
preposicionados y el control de inventario humanitario en zonas
propensas a inundaciones en México, que incorpore \textbf{parámetros
derivados de la incertidumbre en la demanda} (como inventario de
seguridad y penalización por faltantes) y minimice el costo total del
sistema bajo condiciones de \textbf{no convexidad, decisiones mixtas y
...
\item
  \textbf{Formular un modelo de localización--inventario humanitario
  bajo incertidumbre parametrizada}, que extienda el modelo clásico de
...
\item
  \textbf{Evaluar el desempeño del modelo propuesto mediante estudios de
  caso en regiones vulnerables a inundaciones en Chiapas y Veracruz},
  cuantificando la mejora en robustez y costo esperado respecto a
  enfoques deterministas que ignoran la incertidumbre en la demanda o en
  el tiempo de entrega.
\end{enumerate}

\bookmarksetup{startatroot}

\chapter{Preliminares y notación}\label{preliminares-y-notaciuxf3n}

\section{Conjuntos y espacios
vectoriales}\label{conjuntos-y-espacios-vectoriales}
...
\section{Convexidad y conjuntos
especiales}\label{convexidad-y-conjuntos-especiales}
...
\item
  ...
  y convexo (Bazaraa et al., 2013), y describe sistemas de restricciones
  lineales como límites de capacidad o cobertura de demanda.
\end{itemize}
...
\pandocbounded{\includegraphics[keepaspectratio]{chapters/Preliminares_files/figure-pdf/fig-poliedro-output-1.pdf}}
...
\section{Funciones y propiedades
analíticas}\label{funciones-y-propiedades-analuxedticas}
...
\pandocbounded{\includegraphics[keepaspectratio]{chapters/Preliminares_files/figure-pdf/fig-separable-output-1.pdf}}
...
\section{Notación de orden
matricial}\label{notaciuxf3n-de-orden-matricial}

\begin{itemize}
\tightlist
\item
  \textbf{\(A \succ 0\)}: matriz simétrica \(A\) es definida positiva,
  es decir, \(x^\top A x > 0\) para todo \(x \neq 0\). Esta notación es
  útil al analizar la curvatura de funciones cuadráticas en modelos de
  inventario.
\end{itemize}

\section{Formulaciones de optimización
mixta}\label{formulaciones-de-optimizaciuxf3n-mixta}

La integración de decisiones estratégicas (ubicación) y tácticas
(inventario) da lugar a formulaciones mixtas:
...
\section{Teoría de probabilidad, incertidumbre y medida de
Lebesgue}\label{teoruxeda-de-probabilidad-incertidumbre-y-medida-de-lebesgue}

En contextos humanitarios, la demanda es incierta. Para modelarla
rigurosamente, es necesario entender que la \textbf{medida de Lebesgue}
generaliza el concepto de ``volumen'' y permite integrar funciones más
allá del cálculo clásico. Esto sustenta la definición de
\textbf{variables aleatorias absolutamente continuas}, aquellas cuya
probabilidad se expresa mediante una densidad.
...
\item
  \textbf{Varianza}: para una v.a. \(\tilde{X}\),
  \(\operatorname{Var}(\tilde{X}) = \mathbb{E}[(\tilde{X} - \mathbb{E}[\tilde{X}])^2]\).
  La varianza cuantifica la incertidumbre en la demanda; ignorarla (por
...
\pandocbounded{\includegraphics[keepaspectratio]{chapters/Preliminares_files/figure-pdf/fig-normal-perdida-output-1.pdf}}
...
\item
  ...
  otra. Por ejemplo, en la Sección 3.4.4 se muestra que, si se ignora la
  incertidumbre en \(L\), la varianza estimada puede ser
...
puntual en modelos deterministas, como se advierte en la Sección
3.1.2.3, esta simplificación puede ser inadecuada en logística
humanitaria, donde la cola de la distribución (eventos extremos) es
crítica.

\section{Optimización y condiciones de
optimalidad}\label{optimizaciuxf3n-y-condiciones-de-optimalidad}
...
\item
  Esto contrasta fuertemente con una solución EOQ ingenua
...
Sección 3.1.2, la determinación del tamaño óptimo de lote se reduce a un
...
de costo total extendida (3.1), se obtiene la función objetivo efectiva:
...
presentan en Bazaraa, Sherali, y Shetty (2013); Boyd y Vandenberghe
(2004).
...
En la Sección 3.5.1 se formuló el problema de optimización
unidimensional como:
...
\(Q^* \in [1, \infty)\) que minimiza \(f(Q)\). En la Sección 3.5.3 se
...
En la Sección 3.5.2 se demostró que el problema de optimización
unidimensional
...
Como se demostró en la Sección 3.5.3, \(f(Q)\) es estrictamente convexa
...
Sustituyendo la expresión de la derivada obtenida en la Sección 3.6.1,
...
En conjunto, las secciones 3.6.1--3.6.5 establecen que, bajo el enfoque
...
\pandocbounded{\includegraphics[keepaspectratio]{chapters/formulacion_files/figure-pdf/fig-funcion-perdida-output-1.pdf}}
