...
(Porteus (2002), Zipkin (2000)). Bajo los supuestos clásicos de demanda
normal durante el tiempo de entrega, política de revisión continua y
costo lineal por faltante, el costo total esperado puede aproximarse
mediante una función determinista del tamaño de lote \(Q\). Esta
aproximación incorpora los efectos de la incertidumbre a través del
inventario de seguridad \(Z_\alpha \sigma_L\) y la penalización esperada
por faltantes, pero su optimización se realiza como un problema
determinista.
...
\begin{equation}\phantomsection\label{eq-Zux28qux29}{
...
Karush--Kuhn--Tucker(KKT) para garantizar optimalidad global, y exige el
uso de métodos especializados para problemas mixtos entero-continuos.
...
enteras, es bien conocida en la teoría de optimización mixta ver
Bertsekas (1999).
...
\begin{equation}\phantomsection\label{eq-Zux28xysux29}{
Z(x,y,s) 
= 
...
problemas es computacionalmente exigente porque: - no se puede aplicar
directamente la optimización convexa, - los métodos de enumeración son
inviables en problemas de tamaño moderado, - y la no linealidad impide
el uso de técnicas eficientes basadas en planos de corte o relajaciones
lineales exactas.
...
la Sección 2.1.1, este espacio de decisiones es híbrido, ya que combina
variables binarias y continuas, compuesto por dos tipos cualitativamente
distintos de variables:
...
\tightlist
\item
  \textbf{Decisiones estratégicas}, como la apertura de almacenes, que
  son irreversibles una vez implementadas y se modelan mediante
  variables binarias;
\item
...
vectorial convexo, cerrado y de dimensión finita.
...
ecuación (2.3), este término contribuye a la función objetivo mediante
el producto lineal \(f_i x_i\), donde \(x_i \in \{0,1\}\) es la variable
binaria. Se exige\\
\[
...
información geoespacial y modelos de capacidad vial como se presenta en
Daskin (2013).
...
\begin{equation}\phantomsection\label{eq-restriccion}{
...
\item
  ...
  pruebas de existencia, estabilidad numérica y escalado de variables de
  acuerdo con las recomendaciones de Lewis y Overton (2013).
\item
...
teorema de existencia de mínimo en poliedros, el subproblema posee
...
la varianza de la
demanda}{Ejemplo de impacto no lineal de L en la varianza de la demanda}}\label{ejemplo-de-impacto-no-lineal-de-l-en-la-varianza-de-la-demanda}
...
más baja: - \(c = 24~\text{unidades monetarias/unidad}\), -
\(p = 25~\text{unidades monetarias/unidad}\) (penalización marginal
apenas positiva), - \(K = 5000~\text{unidades monetarias}\) (costo fijo
inflado por burocracia percibida).
...
unidimensional}\label{propiedades-analuxedticas-del-problema-unidimensional}

Tras formular la función de costo total extendida en la Sección 2.4 y
derivar su expresión determinista efectiva en la Sección 2.6.1, es
necesario establecer las propiedades analíticas fundamentales del
problema de optimización unidimensional resultante. En esta sección, se
analiza rigurosamente el problema \(\min_{Q \geq 1} f(Q)\), demostrando
la existencia y unicidad del minimizador, así como el comportamiento
asintótico de la función objetivo en los extremos del dominio. Estas
propiedades son esenciales para justificar la aplicabilidad de las
condiciones de optimalidad clásicas y para garantizar la estabilidad de
los métodos numéricos empleados en la resolución.

\subsection{\texorpdfstring{Formulación del problema de optimización:
\(\min_{Q \geq 1} f(Q)\)}{Formulación del problema de optimización: \textbackslash min\_\{Q \textbackslash geq 1\} f(Q)}}\label{formulaciuxf3n-del-problema-de-optimizaciuxf3n-min_q-geq-1-fq}

En el contexto del modelo de inventario extendido presentado en la
Sección 2.1.2, la determinación del tamaño óptimo de lote se reduce a un
problema de optimización escalar en la variable continua \(Q > 0\). Tras
aislar los términos que dependen explícitamente de \(Q\) en la función
de costo total extendida (2.1), se obtiene la función objetivo efectiva:
...
\(K := D(S + \beta C) > 0\), - \(a := H/2 > 0\).

Estos parámetros son estrictamente positivos bajo las hipótesis
operativas estándar (demanda positiva, costos no negativos, penalización
estrictamente positiva), lo cual garantiza que \(f(Q)\) esté bien
definida y sea diferenciable en \((0, \infty)\).
...
sustentadas en resultados clásicos del análisis no lineal (Bazaraa,
Sherali, y Shetty (2013); Boyd y Vandenberghe (2004)).
...
mínimo}\label{continuidad-y-coercividad-garantuxeda-de-existencia-del-muxednimo}

En la Sección 2.5.1 se formuló el problema de optimización
unidimensional como:
...
\(Q^* \in [1, \infty)\) que minimiza \(f(Q)\). En la Sección 2.5.3 se
demostrará que este minimizador es único y se encuentra en el interior
del dominio, ya que \(Q^* \approx 489.9 > 1\).
...
óptimo}\label{anuxe1lisis-de-convexidad-local-y-unicidad-del-uxf3ptimo}

En la Sección 2.5.2 se demostró que el problema de optimización
unidimensional
...
Recordemos que la función objetivo efectiva, definida en la Sección
2.5.1, es:
...
global}, como se demostró en la Sección 2.5.2.
...
A partir de las propiedades analíticas establecidas en la Sección 2.5,
este apartado deriva formalmente las condiciones de optimalidad de
primer orden para el problema unidimensional\\
\[
\min_{Q \geq 1} Z(Q).
\]\\
...
\(\frac{dZ}{dQ}\)}{Derivación formal de \textbackslash frac\{dZ\}\{dQ\}}}\label{derivaciuxf3n-formal-de-fracdzdq}
...
que corresponde a la ecuación (2.1) en este documento. Aunque los
parámetros \(Z_\alpha\) y \(\sigma_L\) se derivan de un análisis
estocástico (stock de seguridad bajo demanda aleatoria), \textbf{el
modelo de optimización es determinista}, conforme al enfoque robusto
adoptado en este trabajo (ver Sección 2.1.1). En consecuencia, \(Z(Q)\)
se trata como una función real diferenciable en \((0,\infty)\), cuyos
parámetros reflejan una parametrización de la incertidumbre, pero no su
modelación explícita.
...
no constante de la ecuación (2.1) como:
...
parámetros numéricos tomados de la Sección 2.1.2:
...
En la Sección 2.6.1 se estableció que la función de costo total
extendida, bajo el enfoque determinista robusto adoptado en este
...
Como se demostró en la Sección 2.5.3, \(f(Q)\) es estrictamente convexa
en \((0, \infty)\). En consecuencia, posee un único minimizador global
en cualquier subconjunto convexo de su dominio. En particular, en el
dominio restringido \([1, \infty)\), el minimizador existe y es único
(Secciones 2.5.2--2.5.3).
...
Sustituyendo la expresión de la derivada obtenida en la Sección 2.6.1,
esta condición se escribe explícitamente como:
...
garantizadas en la Sección 2.5 aseguran que este punto crítico es, de
hecho, el \textbf{único minimizador global} del problema.
...
Retomamos los parámetros del ejemplo de la Sección 2.1.2: - Demanda
esperada: \(D = 1000\), - Costo fijo de pedido: \(S = 100\), - Costo de
mantenimiento: \(H = 5\), - Costo unitario de adquisición: \(C = 10\), -
Penalización por faltante: \(\beta = 50\).
...
propiedades de existencia y unicidad demostradas en la Sección 2.5.
...
\(\beta\), \(Z_\alpha\), \(H\), y
\(S\)}{Sensibilidad del óptimo respecto a \textbackslash beta, Z\_\textbackslash alpha, H, y S}}\label{sensibilidad-del-uxf3ptimo-respecto-a-beta-z_alpha-h-y-s}
...
En conjunto, las secciones 2.6.1--2.6.5 establecen que, bajo el enfoque
determinista robusto adoptado en este trabajo, el problema de
optimización unidimensional \(\min_{Q \geq 1} Z(Q)\) admite una
\textbf{solución cerrada explícita}, dada por la proyección
...
Bazaraa, Mokhtar S., Hanif D. Sherali, y C. M. Shetty. 2013.
