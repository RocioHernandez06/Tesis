...
\emph{Demostración.} Al fijar \(\bar{x}\), los t\textquotesingle erminos
\(f_i \bar{x}_i\) se convierten en constantes. La
funci\textquotesingle on objetivo resultante es lineal en \((y,s)\), y
todas las restricciones que definen \(\mathcal{X}(\bar{x})\) son
lineales en estas variables. Por definici\textquotesingle on, esto
constituye un problema de programación lineal, cuyo conjunto factible es
un poliedro convexo. \(\blacksquare\)
\end{quote}
...
variables discretas y continuas: \textbf{la solución óptima puede
cambiar de manera discontinua ante pequeñas variaciones en los
parámetros} (por ejemplo, si \(f_2\) disminuye a 350, sigue siendo
óptimo; pero si \(f_1\) baja a 250, podría convenir abrir el almacén 1).
Esta \textbf{sensibilidad discreta}, originada por la naturaleza
combinatoria de las decisiones de apertura, es una característica
distintiva de los modelos mixtos entero-continuos (Daskin (2013)).
...
\section{\texorpdfstring{\textbf{Definición formal del espacio factible
...
\section{\texorpdfstring{\textbf{Definición (Espacio
